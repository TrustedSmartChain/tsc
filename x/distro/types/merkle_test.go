...
func TestLeafHashDeterministic(t *testing.T) {
	a := types.LeafHash(1, "tsc1abc", "1000")
	b := types.LeafHash(1, "tsc1abc", "1000")
	require.Equal(t, a, b)
	require.Len(t, a, 32)
}
...
func TestLeafHashFieldBoundaryUnambiguous(t *testing.T) {
	...
	require.NotEqual(t,
		types.LeafHash(1, "ab", "c"),
		types.LeafHash(1, "a", "bc"),
	)
	// Different nonce => different leaf.
	require.NotEqual(t,
		types.LeafHash(1, "tsc1abc", "1000"),
		types.LeafHash(2, "tsc1abc", "1000"),
	)
}

func TestHashPairCommutative(t *testing.T) {
	a := types.LeafHash(1, "a", "1")
	b := types.LeafHash(2, "b", "2")
	require.Equal(t, types.HashPair(a, b), types.HashPair(b, a))
}

func TestVerifyProofSingleLeaf(t *testing.T) {
	leaf := types.LeafHash(0, "tsc1only", "42")
...
func TestVerifyProofFourLeaves(t *testing.T) {
	l0 := types.LeafHash(0, "tsc1a", "10")
	l1 := types.LeafHash(1, "tsc1b", "20")
	l2 := types.LeafHash(2, "tsc1c", "30")
	l3 := types.LeafHash(3, "tsc1d", "40")
...
func TestVerifyProofRejectsTampering(t *testing.T) {
	...
	bad := types.LeafHash(0, "tsc1a", "9999")
